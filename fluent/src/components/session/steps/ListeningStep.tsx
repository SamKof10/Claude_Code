...
import { LISTENING_EXERCISES, ACCENT_LABELS } from "@/lib/content/listening";
import { pickOne } from "@/lib/utils";
import { cn } from "@/components/ui/cn";
import { useRandomOnMount } from "@/hooks";

const DEMO_PLAY_MS = 4500;

export function ListeningStep({ onComplete }: { onComplete: () => void }) {
  const EXERCISE = useRandomOnMount(() => pickOne(LISTENING_EXERCISES), LISTENING_EXERCISES[0]);
  ...
  const allAnswered = Object.keys(answers).length === EXERCISE.questions.length;
  const accent = ACCENT_LABELS[EXERCISE.accent];

  return (
    <StepCard>
      <StepKicker>Listening</StepKicker>
      ...
      <h2 className="mt-3 text-[19px] font-medium text-ink-1">{EXERCISE.title}</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-1 items-end gap-[3px]" aria-hidden>
            {Array.from({ length: 28 }).map((_, i) => (
              <span
                key={i}
                className={cn("wave-bar w-full rounded-full", i / 28 < progress / 100 ? "bg-signal" : "bg-surface-3")}
                style={{ height: `${14 + ((i * 37) % 20)}px`, animationPlayState: playing ? "running" : "paused", animationDelay: `${i * 0.05}s` }}
              />
            ))}
          </div>
        </div>
        ...
        {EXERCISE.questions.map((q, qi) => (
          <div key={qi}>
            ...
            {answers[qi] !== undefined ? (
              <FeedbackBanner correct={answers[qi] === q.answerIndex}>{q.explanation}</FeedbackBanner>
            ) : null}
          </div>
        ))}
      </div>

      <Button onClick={onComplete} disabled={!allAnswered} className="mt-6 w-full" size="lg">
        Continue
      </Button>
    </StepCard>
  );
