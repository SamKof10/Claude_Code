...
export function ChatMessage({ role, content, streaming }: { role: "user" | "assistant"; content: string; streaming?: boolean }) {
  ...
  return (
    <div className={cn("flex gap-3", isUser && "flex-row-reverse")}>
      ...
      <div className={cn("max-w-[75%] rounded-2xl px-4 py-2.5", isUser ? "bg-surface-2 text-ink" : "border border-border bg-surface")}>
        {isUser ? (
          <p className="whitespace-pre-wrap text-[13.5px] leading-relaxed">{content}</p>
        ) : (
          <>
