        ...
        <Link key={subject.id} href={`/subjects/${subject.id}`} className="group flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="mb-1.5 flex items-center justify-between gap-2">
              <span className="truncate t-callout font-medium text-ink group-hover:text-[var(--color-signal-2)] transition-colors">{subject.name}</span>
              <span className="shrink-0 tabular-nums t-caption text-ink-3">{progress}%</span>
            </div>
